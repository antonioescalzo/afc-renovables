...
function parseSheetCSV(text) {
  ...
  // Find header row: first row where first cell looks like a real column key (not a title)
  let headerIdx = lines.findIndex(row =>
    row[0] && row[0].length < 40 && !row[0].startsWith("🏭") &&
    !row[0].startsWith("⚠️") && !row[0].startsWith("📊") &&
    !row[0].startsWith("📥") && !row[0].startsWith("📤") &&
    !row[0].startsWith("📦") && !row[0].startsWith("📋") &&
    !row[0].startsWith("⏱️") && !row[0].startsWith("✏️") &&
    !row[0].startsWith("Registra") && !row[0].startsWith("Cada") &&
    !row[0].startsWith("⚙️") && !row[0].startsWith("IDENTIFICACIÓN")
  );
  if (headerIdx < 0) headerIdx = 0;
  const headers = lines[headerIdx].map(h =>
    h.trim().toLowerCase()
      .replace(/[🔄📊📥📤⚠️]/g, "")
      .replace(/\n/g, " ").replace(/\s+/g, "_")
      .replace(/[€()%º]/g, "").replace(/[^a-záéíóúüñ0-9_]/g, "_")
