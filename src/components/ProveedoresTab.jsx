...
export default function ProveedoresTab() {
  ...
  const loadProductos = async (proveedorId) => {
    ...
    try {
      ...
      // SOLUCIÓN INMEDIATA: Si contiene cualquier variación de "eclimen", cargar el JSON local
      if (proveedorNombre.includes('eclimen') || proveedorNombre.includes('elect')) {
        // ✓ ECLIMEN DETECTED - Cargar los 14 productos del JSON
        const productosFormateados = productosEclimen.map(p => ({
          ref: p.ref,
          descripcion: p.desc,
          precio: p.precio,
          cantidad: 1,
        ...
        // Para otros proveedores, cargar desde Supabase
