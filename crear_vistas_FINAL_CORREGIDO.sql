...
SELECT
  ...
  (MAX(f.fecha)::date - MIN(f.fecha)::date) as dias_activo
...
SELECT
  ...
  CASE
    ...
    ELSE 'Inactivo'
  END as estado,
  (CURRENT_DATE - MAX(f.fecha)::date) as dias_sin_compra
FROM facturas_compra f
JOIN proveedores p ON f.proveedor_id = p.id
WHERE f.csv_origen = 'proveedores.csv'
GROUP BY p.id, p.nombre
...
-- ═══════════════════════════════════════════════════════════════════════════
-- VISTA 9: Distribución de Gasto (para gráfico pastel) - CORREGIDA
-- ═══════════════════════════════════════════════════════════════════════════
CREATE OR REPLACE VIEW v_distribucion_gasto AS
SELECT
  CASE
    WHEN ranking <= 5 THEN proveedor
    ELSE 'Otros'
  END as proveedor,
  SUM(facturas) as facturas,
  ROUND(SUM(gasto_total)::numeric, 2) as gasto_total
FROM (
  SELECT
    ROW_NUMBER() OVER (ORDER BY SUM(f.total_factura) DESC) as ranking,
    p.nombre as proveedor,
    COUNT(f.id) as facturas,
    SUM(f.total_factura) as gasto_total
  FROM facturas_compra f
  JOIN proveedores p ON f.proveedor_id = p.id
  WHERE f.csv_origen = 'proveedores.csv'
  GROUP BY p.id, p.nombre
...
-- ═══════════════════════════════════════════════════════════════════════════
SELECT
  'Vistas creadas correctamente:' as resultado,
  COUNT(*) as cantidad,
  array_agg(table_name) as vistas
